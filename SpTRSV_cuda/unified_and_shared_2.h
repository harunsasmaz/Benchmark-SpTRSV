...
{    
    ...
    VALUE_TYPE xi = s_left_sum[global_x_id + displs] + x_left_sum[local_warp_id] + d_left_sum[global_x_id];
    ...
    for(int j = start_ptr + lane_id; j < stop_ptr; j += WARP_SIZE)
    {
        ...
        const bool cond = row < n + displs;

        if(cond)
        {   
            const bool cond2 = row < starting_x + WARP_PER_BLOCK;
            if(cond2)
            {
                ...
                const int index = row - displs;
                atomicAdd((VALUE_TYPE *)&d_left_sum[index], xi * d_cscVal[j - e_displs]);
                __threadfence();
                atomicAdd((int *)&d_in_degree[index], 1);
            }
                         ...
                         const int            ngpu)
{
    ...
    for(int k = 0; k < BENCH_REPEAT; k++)
    {
        ...
        for(int i = 0; i < ngpu; i++)
        {
            ...
            unified_and_shared_2_executor<<< num_blocks, num_threads >>>
    ...
    printf("unified+shared 2 SpTRSV solve used %4.2f ms, throughput is %4.2f gflops\n",
    ...
    if (res < accuracy)
        printf("unified+shared 2 SpTRSV executor passed! |x-xref|/|xref| = %8.2e\n", res);
    else
        printf("unified+shared 2 SpTRSV executor _NOT_ passed! |x-xref|/|xref| = %8.2e\n", res);
